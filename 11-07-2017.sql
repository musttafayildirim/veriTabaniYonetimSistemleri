...
and Uye.Uye_No IN (
select s2.Uye_No from (select s1.Uye_No as Uye_No, COUNT(s1.Uye_No) as stno_say 
from (select Uye.Uye_No as Uye_No, Soru_Tur.Soru_Tür_Adi, COUNT(Soru_Tur.Soru_Tur_No) as a from Soru,Uye,Soru_Tur
where soru.Uye_No=Uye.Uye_No and  Soru_Tur.Soru_Tur_No = Soru.Soru_Tur_No group by Uye.Uye_No, Soru_Tur.Soru_Tür_Adi) s1 group by s1.Uye_No having (Count(s1.Uye_No)<=2)) s2) 
